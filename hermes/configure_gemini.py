...
# Unchanged from the previous configuration: free, 1M context, and the tool
# surface alone is ~6.9k tokens per call, so a small context is not an option.
...
def main() -> int:
    ...
    if mode == "measure" and not keep_fallback:
        ...
        print(f"  fallback_providers -> {FALLBACK_PROVIDER}/{FALLBACK_MODEL}")
